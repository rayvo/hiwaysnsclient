...
public class ElveXmlProc {
	...
	public String appendField(String fieldName, String fieldValue) {
		mXmlData = mXmlData + "<" + fieldName + ">" + fieldValue + "</"
				+ fieldName + ">";
		return mXmlData;
	}
	...
	public String[][] parseInputXML(String strInputXml, String[][] inputList) {
		try {
			 ...
			 * ); Document doc = builder.parse( is ); NodeList troasis =
			 * doc.getElementsByTagName("troasis"); NodeList channel =
			 * troasis.item(0).getChildNodes(); int countList =
			 * (int)channel.getLength(); String tagName; for ( int i = 0; i <
			 * countList; i++ ) { tagName = channel.item(i).getNodeName(); if (
			 * tagName.length() < 1 || tagName.getBytes()[0] == '#' ) continue;
			 * for ( int j = 0; j < inputList.length; j++ ) { if (
			 * tagName.compareToIgnoreCase(inputList[j][0]) == 0 )
			...
			// InputSource is = new InputSource( new StringReader(strInputXml)
			// );
			ByteArrayInputStream is = new ByteArrayInputStream(strInputXml
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.START_TAG:
					...
					for (int j = 0; j < inputList.length; j++) {
						if (tagName.equalsIgnoreCase(inputList[j][0])) {
							...
							// Log.i( "[FIELD] ","Field=" + tagName + ", Value="
							// + strValue );
						}
					}
					break;
				...
				case XmlPullParser.TEXT:
					// Log.i( "[ANDROID] ","Text : " + tagName + " : " +
					// xpp.getText() );
					break;

				default:
					// Log.i( "[ANDROID] ","Else : " + tagName + " With : " +
					// eventType );
					break;
				}
	...
	// GET 또는 POST 방식으로 수신된 XML 입력정보 파싱.
	public List<String[]> parseMemberXML(String strInputXml, String subMember,
			String[] listMember) {
		// 입력인자 목록의 데이터 초기화.
